import type { MediaItem, Product, PublicRoute, PublicStore, SocialLink, WallItem } from './types.js';
...
export function isFileProduct(product: Product): boolean {
  return String(product.ptype || '').toLowerCase() === 'file';
}
...
function normalizeStore(payload: unknown): PublicStore {
  ...
  const contentEmail = typeof content.contactEmail === 'string' && content.contactEmail.includes('@') ? content.contactEmail : '';
  const legacyEmail = typeof source.contactEmail === 'string' && source.contactEmail.includes('@') ? source.contactEmail : '';
  return {
    products: products.filter((product) => product.active !== false),
    gallery: normalizeGallery(source.gallery),
    content,
    contactEmail: contentEmail || legacyEmail || DEFAULT_CONTACT,
  };
}
...
export function parseSocials(value: unknown): SocialLink[] {
  ...
  return value.flatMap((line) => {
    ...
    try {
      ...
      return [{ label: rawLabel, url: url.toString() }];
    } catch (_error) {
      return [];
    }
  });
  ...
  return `
    ...
    <header class="topbar">
      ...
      <nav class="desktop-nav" aria-label="Primary navigation">
        ...
        <a class="nav-link${activeClass('contact', route)}" href="/contact/">Contact</a>
      </nav>
...
function caseNumber(product: Product): string {
...
function productCard(product: Product, index = 0): string {
  ...
  const isFile = isFileProduct(product);
  return `<a class="product-card reveal${isFile ? ' product-card--file' : ''}" style="--delay:${Math.min(index, 8) * 45}ms" href="${productPath(product)}">
    <div class="product-card-media">
      ...
      ${isFile ? `<span class="file-stamp">EYES ONLY</span><span class="file-case">CASE // TF-${caseNumber(product)}</span>` : ''}
...
function renderFiles(store: PublicStore): void {
  ...
  const content = `<section class="route-intro compact-intro files-intro"><h1 class="sr-only">The Files</h1><span class="eyebrow files-eyebrow">Classified // Operator Intel</span><p>${escapeHtml(store.content.filesSub || 'Field-tested operations, documented and sealed.')}</p></section>
    ...
    ${products.length ? `<div class="dossier-rail">${products.map((product, index) => {
      const media = mediaFor(product)[0];
      return `<a class="dossier-card${index === 0 ? ' is-active' : ''}" href="${productPath(product)}">
        <div class="dossier-bar"><span>Case // TF-${caseNumber(product)}</span></div>
        <div class="dossier-media">${media?.type !== 'video' && media?.url ? `<img src="${escapeHtml(media.url)}" alt="${escapeHtml(product.name)}" loading="lazy">` : `<b>${escapeHtml(product.name.slice(0, 2).toUpperCase())}</b>`}</div>
        <div class="dossier-body">
          <h3>${escapeHtml(product.name)}</h3>
          <p>${escapeHtml((product.desc || '').slice(0, 120))}</p>
          <div class="dossier-foot"><strong>${formatMoney(product.price)}</strong><span>View file →</span></div>
        </div>
      </a>`;
    }).join('')}</div><div class="rail-dots" aria-label="Choose file">${products.map((product, index) => `<button type="button" class="${index === 0 ? 'is-active' : ''}" aria-label="Show ${escapeHtml(product.name)}"></button>`).join('')}</div>` : emptyState('New files will appear here once declassified.')}</section>
    ${trustStrip(store.content.strip || [])}`;
  app.innerHTML = chrome('files', content, store);
  bindChrome();
  const filesRail = document.querySelector<HTMLElement>('.dossier-rail');
  if (filesRail) installRail(filesRail, '.dossier-card');
  installReveals();
  setMeta('The Files — ShadowGLB', 'Classified operator intel and field-tested operations from ShadowGLB.');
}

export interface ReviewItem { name: string; text: string; image?: string; }
export function parseReviews(value: unknown): ReviewItem[] {
  if (!Array.isArray(value)) return [];
  return value.flatMap((line) => {
    if (typeof line !== 'string') return [];
    const parts = line.split('|').map((part) => part.trim());
    const [name, text, imageUrl] = parts;
    if (!name || !text) return [];
...
function renderWall(store: PublicStore): void {
  ...
  const content = `<section class="route-intro wall-intro"><h1 class="sr-only">The Wall</h1><span class="eyebrow">The Wall // Visual archive</span><p>${escapeHtml(store.content.wallSub || 'A visual record. Tap any frame to enter.')}</p></section>
    ...
    ${parseReviews(store.content.reviews).length ? `<section class="reviews-wrap section-wrap" aria-label="Customer reviews"><div class="section-heading"><div><span>Field reports</span><h2>What operators say.</h2></div></div><div class="review-grid">${parseReviews(store.content.reviews).map((review) => `<figure class="review-card reveal">${review.image ? `<img src="${escapeHtml(review.image)}" alt="Review from ${escapeHtml(review.name)}" loading="lazy">` : ''}<blockquote>${escapeHtml(review.text)}</blockquote><figcaption>${escapeHtml(review.name)}</figcaption></figure>`).join('')}</div></section>` : ''}
...
function renderContact(store: PublicStore): void {
  if (!app) return;
  ...
  const content = `<section class="route-intro"><h1 class="sr-only">Contact</h1><span class="eyebrow">Contact // Reach the operator</span><p>Real questions get real answers. Pick whichever works for you.</p></section>
    <section class="contact-wrap">
      <a class="contact-card reveal" href="mailto:${escapeHtml(email)}">
      ...
      ${socials.length ? `<div class="contact-socials reveal">
        ...
        <div class="contact-social-links">${socials.map((item) => `<a class="button button-ghost" href="${escapeHtml(item.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(item.label)} <b>↗</b></a>`).join('')}</div>
      </div>` : ''}
    </section>`;
...
async function boot(): Promise<void> {
  ...
  const renderRoute = (store: PublicStore): void => {
    ...
    if (route === 'contact') renderContact(store);
